...
/*
 * 插入排序（Insertion Sort）時間複雜度：
 *   最佳情況：O(n)
 *   平均情況：O(n^2)
 *   最差情況：O(n^2)
 * 解釋：
 *   - 最佳：如果資料本身是遞增排序，內部比較只需執行 n-1 次 → 屬線性時間。
 *   - 平均與最差：每當插入一個元素時，可能需將前面資料逐一後移，整體約需進行 (n^2)/2 次比較與移動。
 */

/*
 * 二分插入排序（Binary Insertion Sort）時間複雜度：
 *   最佳情況：O(n log n)
 *   平均情況：O(n^2)
 *   最差情況：O(n^2)
 * 解釋：
 *   - 最佳：透過二分搜尋法可快速定位插入點，每次操作為 O(log n)，共需 n 次插入。
 *   - 平均與最差：儘管能快速找到位置，但移動元素的成本仍存在，最多每次需搬移 O(n) 個元素，因此整體仍為 O(n^2)。
 */

...
/*
 * 泡沫排序（Bubble Sort）時間複雜度：
 *   最佳情況：O(n)
 *   平均情況：O(n^2)
 *   最差情況：O(n^2)
 * 解釋：
 *   - 最佳情況下，若資料本來就是有序的，只需進行一次完整比較，無需交換即可結束 → 時間為線性。
 *   - 平均與最壞狀況時，每一輪都要比較相鄰元素並可能交換，總共會進行約 n(n-1)/2 次比較，為平方等級。
 */

/*
 * 雞尾酒排序（Cocktail Sort）時間複雜度：
 *   最佳情況：O(n)
 *   平均情況：O(n^2)
 *   最差情況：O(n^2)
 * 解釋：
 *   - 在理想狀況下，資料已排好序，從左到右與從右到左各走一次就能確認無需交換 → 運行時間為 O(n)。
 *   - 一般或最糟狀態中，需反覆往返檢查並交換資料，與傳統泡沫排序相同會進行 O(n^2) 次比較與交換。
 */
